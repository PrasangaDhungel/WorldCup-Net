"""network2.py
Implementing the stochasticgradient descent learning algorithm for a feedforward neural network.
Improvements include the addition of the cross-entropy cost function,regularization, and better 
initialization of network weights. It is not optimized, and omits many desirable features.
"""
# Standard libraries
import json
import random
...
class CrossEntropyCost(object):
    ...
    @staticmethod
    def fn(a, y):
        """Return the cost associated with an output a and desired output y. np.nan_to_num is used to
        ensure numerical stability.  In particular, if both `a` and `y` have a 1.0 in the same slot,
        then the expression (1-y)*np.log(1-a) returns nan.  The np.nan_to_num ensures that that is converted
...
class Network(object):
    ...
    def __init__(self, sizes, cost=CrossEntropyCost):
        ...
        the second layer 3 neurons, and the third layer 1 neuron.  The biases and weights for the network are initialized
    ...
    def default_weight_initializer(self):
        ...
        The first layer is assumed to be an input layer, and by convention we won't set any biases for those neurons, since
    ...
    def large_weight_initializer(self):
        """Initialize the weights using a Gaussian distribution with mean 0 and standard deviation 1.  Initialize the biases
        using a Gaussian distribution with mean 0 and standard deviation 1. Note that the first layer is assumed to be an
        input layer, and by convention we won't set any biases for those neurons, since biases are only ever used in 
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
        ...
        or the training data, by setting the appropriate flags.  The method returns a tuple containing four lists: the
        (per-epoch) costs on the evaluation data, the accuracies on the evaluation data, the costs on the training
        data, and the accuracies on the training data.  All values are evaluated at the end of each training epoch.  So,
        for example, if we train for 30 epochs, then the first element of the tuple will be a 30-element list containing
        the cost on the evaluation data at the end of each epoch. Note that the lists are empty if the corresponding flag
        is not set.
        """
    ...
    def update_mini_batch(self, mini_batch, eta, lmbda, n):
        ...
        mini batch.  The `mini_batch` is a list of tuples `(x, y)`, `eta` is the learning rate, `lmbda` is the
    ...
    def total_cost(self, data, lmbda, convert=False):
        ...
        training data (the usual case), and to True if the data set is the validation or test data.  See comments on the 
    ...
    def evaluate(self, data):
        """Evaluate the output for given input
        """
        a = [np.argmax(self.feedforward(x)) for (x, y) in data]
        return a
...
def vectorized_result(j):
    """Return a 10-dimensional unit vector with a 1.0 in the j'th position and zeroes elsewhere.  This is used to convert a
